...
// --- Safetensors helpers: slice tensors out of the serialized weight file ---
...
// --- Compute pass helper used throughout the generated inference graph ---
...
// --- Auto-generated WGSL kernels exported by tinygrad (one per compute op) ---
...
// --- Main graph builder: allocates buffers, pipelines, and returns the callable net() ---
const setupNet = async (device, safetensor) => {
    ...
    // Buffer allocations (weights + intermediates) exported from the tinygrad graph.
...
// --- Public entry point: stream weights, build graph, and return inference fn ---
const loadNet = async (device, onProgress) => {
  const response = await fetch('net.safetensors');
  if (!response.ok) {
  ...
  if (!response.body || !response.body.getReader) {
    ...
    return setupNet(device, new Uint8Array(buffer));
  }
  ...
  if (onProgress && totalBytes && receivedBytes >= totalBytes) {
    onProgress(totalBytes, totalBytes);
  }

  return setupNet(device, modelBytes);
}
